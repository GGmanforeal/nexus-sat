...
const Icons = {
  ...
  diagnostic: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="22 12 18 12 15 21 9 3 6 12 2 12"/></svg>,
...
// Desktop nav
const DESKTOP_LINKS = [
  { href: '/diagnostic', label: 'Diagnostic',    icon: 'diagnostic' as const },
...
const NexusLogo = () => (
  <svg width="100" height="40" viewBox="0 0 220 88" fill="none">
  ...
  return (
    <>
      ...
      <nav style={{ height:'var(--nav-h)', display:'flex', alignItems:'center', padding:'0 14px', borderBottom:'1px solid var(--line)', background:'var(--sf)', position:'fixed', top:0, left:0, right:0, zIndex:400 }}>
        ...
        <div className="nav-links" style={{ display:'flex', alignItems:'center', gap:1, flex:1, overflowX:'auto', scrollbarWidth:'none' } as React.CSSProperties}>
          {DESKTOP_LINKS.map(({ href, label, icon }) => {
            ...
            return (
              <Link key={href} href={href} style={{ display:'flex', alignItems:'center', gap:5, padding:'5px 9px', borderRadius:7, fontSize:12.5, fontWeight:active?600:500, color:active?'var(--lime-dk)':'var(--tx3)', background:active?'var(--lime-dim)':'none', textDecoration:'none', whiteSpace:'nowrap', transition:'background .12s,color .12s', flexShrink:0 }}>
                <span style={{ opacity:active?1:0.65, display:'flex' }}>{Icons[icon]}</span>
                {label}
              </Link>
            )
          })}
        ...
        <div style={{ display:'flex', alignItems:'center', gap:6, marginLeft:'auto', flexShrink:0 }}>
          <button onClick={toggleTheme} style={{ width:34, height:34, borderRadius:8, border:'1px solid var(--line2)', background:'var(--sf2)', display:'flex', alignItems:'center', justifyContent:'center', cursor:'pointer', color:'var(--tx2)' }}>
            {theme === 'dark' ? Icons.moon : Icons.sun}
          </button>

          {mounted && (user ? (
            <div ref={dropRef} style={{ position:'relative' }}>
              <button onClick={() => setDropOpen(o => !o)} style={{ width:34, height:34, borderRadius:'50%', background:'var(--lime)', color:'#060a0e', border:'none', fontSize:12, fontWeight:900, cursor:'pointer', display:'flex', alignItems:'center', justifyContent:'center' }}>
                {initials}
              </button>
              {dropOpen && (
                <div style={{ position:'absolute', right:0, top:42, width:220, background:'var(--sf)', border:'1px solid var(--line2)', borderRadius:14, boxShadow:'0 12px 40px rgba(0,0,0,.4)', zIndex:600, overflow:'hidden' }}>
                  ...
                  {[
                    ...
                    <Link key={item.href} href={item.href} onClick={() => setDropOpen(false)} style={{ display:'flex', alignItems:'center', gap:10, padding:'10px 16px', fontSize:13.5, color:'var(--tx2)', textDecoration:'none', borderBottom:'1px solid var(--line)' }}>
                      <span style={{ color:'var(--tx3)', flexShrink:0 }}>{item.icon}</span>{item.label}
                    </Link>
                  ))}
            ...
            <div className="nav-auth-btns" style={{ display:'flex', gap:6 }}>
              <Link href="/login" style={{ padding:'6px 12px', borderRadius:8, fontSize:13, fontWeight:600, color:'var(--tx2)', border:'1px solid var(--line2)', background:'none', textDecoration:'none', whiteSpace:'nowrap' }}>Log in</Link>
              <Link href="/login?signup=1" style={{ padding:'6px 12px', borderRadius:8, fontSize:13, fontWeight:700, color:'#060a0e', background:'var(--lime)', border:'none', textDecoration:'none', whiteSpace:'nowrap' }}>Sign up</Link>
            </div>
          ))}
        </div>
      </nav>
      ...
      <nav className="mob-bottom-nav" style={{ position:'fixed', bottom:0, left:0, right:0, height:64, background:'var(--sf)', borderTop:'1px solid var(--line)', zIndex:500, alignItems:'stretch', justifyContent:'space-around' }}>
        {[
          ...MOBILE_LINKS,
          { href: user ? '/profile' : '/login', label: user ? (user.name?.split(' ')[0]?.slice(0,6) || 'Me') : 'Account', icon: 'account' as const },
        ].map(({ href, label, icon }) => {
