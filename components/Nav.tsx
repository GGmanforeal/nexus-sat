...
const LINKS = [
  { href: '/bank',     label: 'Question Bank', shortLabel: 'Bank',     icon: 'bank'     as const },
  { href: '/tests',    label: 'Tests',         shortLabel: 'Tests',    icon: 'tests'    as const },
  { href: '/stats',    label: 'Stats',         shortLabel: 'Stats',    icon: 'stats'    as const },
  { href: '/saved',    label: 'Saved',         shortLabel: 'Saved',    icon: 'saved'    as const },
  { href: '/mistakes', label: 'Mistakes',      shortLabel: 'Mistakes', icon: 'mistakes' as const },
  { href: '/score',    label: 'Score',         shortLabel: 'Score',    icon: 'score'    as const },
]
...
// ── Logo as inline SVG (recreated from the green orbit + NEXUS text) ──────
const NexusLogo = () => (
...
export function Nav() {
  ...
  return (
    <>
      ...
      {/* ── MOBILE BOTTOM NAV ── */}
      <nav className="mob-bottom-nav" style={{ display:'none', position:'fixed', bottom:0, left:0, right:0, height:64, background:'var(--sf)', borderTop:'1px solid var(--line)', zIndex:500, alignItems:'stretch', justifyContent:'space-around' }}>
        {[...LINKS, { href: user ? '/profile' : '/settings', shortLabel: user ? (user.name?.split(' ')[0] || 'Me') : 'Account', icon: 'account' as const, label:'' }]
          .map(({ href, shortLabel, icon }) => {
            ...
            return (
              <Link key={href} href={href} style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:3, textDecoration:'none', flex:1, minWidth:0, color: active ? 'var(--lime-dk)' : 'var(--tx3)', background: active ? 'var(--lime-dim)' : 'transparent', borderTop: active ? '2px solid var(--lime-dk)' : '2px solid transparent', transition:'color .15s, background .15s' }}>
                ...
                <span style={{ fontSize:9.5, fontWeight: active ? 700 : 400, whiteSpace:'nowrap', overflow:'hidden', textOverflow:'ellipsis', maxWidth:'100%', paddingInline:2 }}>{shortLabel}</span>
              </Link>
            )
          })}
      </nav>
