...
const IcTrophy = () => <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="var(--lime-dk)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6"/><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18"/><path d="M4 22h16"/><path d="M10 14.66V17c0 .55-.47.98-.97 1.21C7.85 18.75 7 20.24 7 22"/><path d="M14 14.66V17c0 .55.47.98.97 1.21C16.15 18.75 17 20.24 17 22"/><path d="M18 2H6v7a6 6 0 0 0 12 0V2z"/></svg>

const MISSION_ICONS: Record<string, () => React.ReactElement> = {
  target: IcTarget, rocket: IcRocket, bolt: IcBolt, book: IcBook, chart: IcChart,
}
...
function getDailyMissions(
  ...
  if (weakSkills.length > 0) {
    missions.push({ id: 'weak-drill', icon: 'target', color: '#f472b6',
      ...
      action: `/bank?skill=${encodeURIComponent(weakSkills[0].skill)}`, actionLabel: 'Start drill', xp: 150 })
  } else if (totalAnswered === 0) {
    missions.push({ id: 'first-questions', icon: 'rocket', color: 'var(--lime-dk)',
      title: 'Answer your first 10 questions',
      desc: 'Get started in the Question Bank. Every expert was once a beginner.',
      action: '/bank', actionLabel: 'Open bank', xp: 100 })
  }

  const domains = ['Algebra', 'Standard English Conventions', 'Information and Ideas', 'Advanced Math']
  missions.push({ id: 'timer-challenge', icon: 'bolt', color: '#60a5fa',
    title: 'Speed round: ' + domains[seed % domains.length],
    desc: '5 questions with the 75-second timer on. Train your brain to think under pressure.',
    action: `/bank?domain=${encodeURIComponent(domains[seed % domains.length])}`, actionLabel: 'Start timed', xp: 120 })

  if (totalAnswered > 5) {
    missions.push({ id: 'review', icon: 'book', color: '#a78bfa',
      title: 'Review your mistakes',
      desc: 'Go through every question you got wrong. Understanding errors is how scores actually improve.',
      action: '/mistakes', actionLabel: 'Review now', xp: 80 })
  } else {
    missions.push({ id: 'diagnostic', icon: 'chart', color: '#fb923c',
      ...
      action: '/diagnostic', actionLabel: baselineScore ? 'Continue' : 'Take now', xp: 200 })
  }
...
export default function HomePage() {
  ...
  if (user) {
    ...
    const predicted    = sessionStore.getPredictedScore()
    const currentScore = predicted.total || baseline
      ...
      <div style={{ maxWidth: 920, margin: '0 auto', padding: '24px 20px 100px' }}>
        ...
        <div style={{ marginBottom: 20 }}>
          ...
          {currentScore && target ? (
            <div style={{ fontSize: 14, color: 'var(--tx3)' }}>
              ...
              {' '}&rarr; targeting <span style={{ color: 'var(--lime-dk)', fontWeight: 700 }}>{target}</span>
              {pointGap && pointGap > 0 && <span style={{ color: 'var(--tx4)' }}> &middot; {pointGap} points to go</span>}
            </div>
          ) : (
            <div style={{ fontSize: 14, color: 'var(--tx3)' }}>
        ...
        {currentScore && target && (
          <div style={{ background: 'var(--sf)', border: '1px solid var(--line)', borderRadius: 16, padding: '16px 20px', marginBottom: 16 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
              ...
              <Link href="/score" style={{ fontSize: 12, color: 'var(--lime-dk)', fontWeight: 600, textDecoration: 'none' }}>Full predictor &rarr;</Link>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between' }}>
              ...
              <span style={{ fontSize: 12, fontWeight: 700, color: 'var(--lime-dk)', display: 'flex', alignItems: 'center', gap: 4 }}>
                Target: {target} <IcTrophy />
              </span>
            </div>
          </div>
        )}
        ...
        <div style={{ background: 'var(--sf)', border: '1px solid var(--line)', borderRadius: 14, padding: '14px 18px', marginBottom: 16, display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <div style={{ flex: 1, minWidth: 140 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 4 }}>
              ...
              {!levelInfo.isMax && <span style={{ fontSize: 10.5, color: 'var(--tx4)', fontFamily: 'var(--mono)' }}>&rarr; {levelInfo.next.name}</span>}
            </div>
        ...
        {stats.total > 0 && (
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 10, marginBottom: 16 }}>
            {[
              { label: 'Answered', val: String(stats.total),   color: 'var(--lime-dk)' },
              { label: 'Accuracy', val: `${stats.acc}%`,       color: stats.acc >= 70 ? 'var(--g-tx)' : stats.acc >= 50 ? 'var(--a-tx)' : 'var(--r-tx)' },
              { label: 'Correct',  val: String(stats.correct), color: 'var(--g-tx)' },
            ].map(s => (
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(140px,1fr))', gap: 8 }}>
          {[
            ...
            <Link key={a.href} href={a.href} style={{ textDecoration: 'none', display: 'block', padding: '12px 14px', background: 'var(--sf)', border: '1px solid var(--line)', borderRadius: 12, fontSize: 13, fontWeight: 600, color: 'var(--tx2)', borderLeft: `3px solid ${a.color}` }}>
              {a.label}
            </Link>
          ))}
        </div>
    ...
    <div>
      ...
      <section style={{ maxWidth: 860, margin: '0 auto', padding: 'clamp(60px,10vw,100px) 20px clamp(40px,6vw,60px)', textAlign: 'center' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 12 }}>
          ...
          <div style={{ fontSize: 13, color: 'var(--tx4)' }}>Free diagnostic &rarr; instant score &rarr; personal plan</div>
        </div>
      </section>
      ...
      <section style={{ background: 'var(--sf)', borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)', padding: 'clamp(40px,6vw,64px) 20px' }}>
        <div style={{ maxWidth: 820, margin: '0 auto' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 16 }}>
            {[
              { num: '01', color: '#f472b6', bg: 'rgba(244,114,182,.1)', title: 'Know your baseline', body: 'Take the 10-question diagnostic. In 8 minutes you\'ll have your real score estimate and know exactly what\'s holding you back.', cta: 'Take diagnostic', href: '/diagnostic' },
              { num: '02', color: 'var(--lime-dk)', bg: 'rgba(163,230,53,.1)', title: 'Follow your daily plan', body: 'Every day you get 3 targeted missions — drill your weakest topics, do a timed challenge, review mistakes. 20 minutes. Real progress.', cta: 'Sign up free', href: '/settings' },
              { num: '03', color: '#60a5fa', bg: 'rgba(96,165,250,.1)', title: 'Watch your score climb', body: 'Your predicted score updates live after every answer. See yourself close the gap to your target. The progress is visible. It\'s motivating.', cta: 'View score predictor', href: '/settings' },
            ].map(s => (
        ...
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          ...
          {!qRevealed && !qTiming && (
            <button onClick={() => { setQTiming(true); setQTimer(75) }}
              ...
              <Clock /> Enable pressure timer (75s)
            </button>
          )}
        </div>
        ...
        <div style={{ background: 'var(--sf)', border: `2px solid ${qUrgent ? 'var(--r-tx)' : 'var(--line)'}`, borderRadius: 20, padding: 'clamp(20px,4vw,32px)', boxShadow: '0 12px 50px rgba(0,0,0,.2)', animation: qUrgent ? 'pulseRed 1s infinite' : 'none' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 18 }}>
            {SAMPLE_Q.choices.map((opt, i) => {
              const border = !qRevealed ? (qSel === i ? 'var(--lime)' : 'var(--line)') : i === SAMPLE_Q.answer ? 'var(--g-tx)' : qSel === i ? 'var(--r-tx)' : 'var(--line)'
              const bg     = !qRevealed ? (qSel === i ? 'var(--lime-dim)' : 'var(--sf)') : i === SAMPLE_Q.answer ? 'var(--g-bg)' : qSel === i ? 'var(--r-bg)' : 'var(--sf)'
              const lblBg  = !qRevealed ? (qSel === i ? 'var(--lime)' : 'transparent') : i === SAMPLE_Q.answer ? 'var(--g-tx)' : qSel === i ? 'var(--r-tx)' : 'transparent'
          ...
          {!qRevealed ? (
            <button onClick={() => { if (qSel !== null) { setQRevealed(true); setQTiming(false) } }} disabled={qSel === null}
              style={{ padding: '11px 26px', background: 'var(--lime)', color: '#060a0e', border: 'none', borderRadius: 10, fontSize: 14, fontWeight: 800, cursor: qSel !== null ? 'pointer' : 'default', opacity: qSel !== null ? 1 : 0.35 }}>
              Confirm answer
            </button>
          ) : (
            <div>
      ...
      <section style={{ background: 'var(--sf)', borderTop: '1px solid var(--line)', borderBottom: '1px solid var(--line)', padding: 'clamp(40px,6vw,64px) 0' }}>
        <div style={{ maxWidth: 600, margin: '0 auto', padding: '0 16px' }}>
          ...
          <p style={{ textAlign: 'center', fontSize: 10.5, color: 'var(--tx4)', marginTop: 8 }}>CB = College Board &middot; Prep = Prepscholar free tier</p>
        </div>
      </section>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(220px,1fr))', gap: 16 }}>
          {[
            ...
            <div key={t.name} style={{ background: 'var(--sf)', border: '1px solid var(--line)', borderRadius: 18, padding: '22px 20px' }}>
              ...
              <p style={{ fontSize: 13.5, color: 'var(--tx2)', lineHeight: 1.7, marginBottom: 16 }}>&ldquo;{t.text}&rdquo;</p>
